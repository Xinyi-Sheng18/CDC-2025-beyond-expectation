...
# Baseline mean reward is obtained using standard 2-step Q-learning in another file 
...
# Plot baseline and mean curve
plt.axhline(y=baseline, color='red', linestyle='--', label='Conventional RL baseline')
plt.plot(lambda_values, mean_values, marker='o', color='black', linestyle='-', label='Mean score')
...
tikzplotlib.save(f"Lunar_{MULTI_STEP}step_mean.tex")
plt.savefig(f"Lunar_{MULTI_STEP}step_mean.png")
plt.show()
